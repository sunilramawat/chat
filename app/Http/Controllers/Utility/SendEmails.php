...
final class SendEmails{
	...
	public static  function sendContact($to,$email,$phone,$c_id,$name,$msg){

		//$email_record = EmailContent::where('ec_key','=','FORGOT_PASSWORD')->first();
		
		$subject = 'Contact to Hopple Admin?';
	...
	public static  function sendRestPasswordEmail($to,$message,$name,$userId,$code){
		...
		public static  function sendAdminEmail($to,$message,$name){
			//echo '<pre>'; print_r($to); exit;
			//$link  = url('admin-activation').'/'.$userId.'/'.$code;
			
			$subject = 'Enquiry Mail';
			$content = "<h3 style='display: block;width: 100%; padding: 5px 0 15px; font-size: 18px; font-weight: 500;color:#000000; margin: 0;line-height: 21px; text-align: center;'>Title - New Enquire Received</h3><p><h4>{$name}</h4></p><p><h4>{$to}<br><br></h4></p><p style='-webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; mso-table-lspace: 0pt; mso-table-rspace: 0pt; font-family: 'Roboto, sans-serif;font-size: 14px; color:#000000;margin: 0;padding: 0 0 5px; line-height: 21px;display: block;width: 100%;'>Messsage : {$message}.</p><p><br><br>Thanks<br>Team Ghost Tech</p>";
			$content = str_replace("{{receiver}}", '<b>' . $name . '</b>', $content);
			$content = str_replace("{{link}}", '<a href="'.$to.'">' . $to . '</a>', $content);
			$data = array('message'=> $content,'subject' => $subject);
			//echo '<pre>'; print_r($data); exit;
			$SendMail = Mail::to("gh0st.tech.official@gmail.com")->send(new sendmail($data));
		}
